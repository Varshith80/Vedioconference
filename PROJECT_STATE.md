...
**Phase 2 — Marketing & Onboarding** → **Sprint 13 (Feature A First free 60-minute session) released at commit 53e07f1 / tag v1.13.0-phase2-sprint-13; Sprint 13 Feature B (Pack 10 admin grants + €35/unused-session refund + 6-month expiry) implementation complete locally, audit passed the 13 user-locked business rules, release-preparation complete — awaiting explicit user approval before Feature B Git commit / tag / push and before any production/operator action. Feature D has NOT started.**

## Current status

🟢 **Sprint 13 — Feature B (Pack 10 admin grants + €35/unused-session refund + 6-month expiry + TASK 2.1 financial-consistency repair) is implementation-complete locally and release-prepared.** Four new admin-only API routes (`GET /api/admin/pack-grants`, `GET /api/admin/pack-grants/[id]`, `GET /api/admin/pack-grants/[id]/refund-preview`, `POST /api/admin/pack-grants/[id]/refund`), two new admin pages (`/admin/packs`, `/admin/packs/[id]`), one new client component (`PackRefundCard`), and 131 new test cases across 5 files. Two forward-only idempotent migrations applied to LOCAL Supabase only:

- `supabase/migrations/20260913000004_pack_refund_admin_oversight.sql` — `session_grants_update_admin` policy (admin UPDATE gated by `public.is_admin()`, `with check` constrains `status` to `enrollment_status` values), `session_grants_refund_in_bounds` CHECK (four invariants: `refunded_amount_cents ∈ [0, amount_cents]` and `refunded_amount_cents = 0 ⟺ refunded_at IS NULL`), `fn_enrollments_refund()` cascade trigger tightened to `WHERE sg.status IN ('active', 'completed')` so re-entry is idempotent.
- `supabase/migrations/20260914000001_n8n_executions_admin_refund.sql` — `n8n_executions_admin_insert` policy + `n8n_executions_admin_update` policy, both gated by `public.is_admin()`. No DELETE policy (append/audit-only).

**TASK 2.1 financial-consistency repair:** the admin refund route NEVER writes `session_grants.status='refunded'` or `session_grants.refunded_amount_cents`. Those columns are owned SOLELY by the Stripe `charge.refunded` → `payments.status='refunded'` → `fn_enrollments_refund` cascade. The admin route is now an outbox enqueuer only — it records the outbound attempt in `n8n_executions` (with `run_id = refund_request_id`, UNIQUE = at-most-once primitive) and POSTs to `N8N_ENROLLMENT_WEBHOOK_URL`. HTTP responses reflect the n8n outcome: 200 `n8n_accepted` (n8n 2xx — does NOT claim Stripe confirmation), 502 `pack_refund_webhook_failed` (n8n 5xx / throw — outbox row is `failed`), 503 `pack_refund_webhook_unavailable` (webhook URL unset — outbox row is `started`). 409 `pack_refund_already_refunded` also fires on the SQLSTATE 23505 / 23514 race (outbox UNIQUE / DB CHECK). **Audit passed all 13 user-locked business rules (R1–R13):** Pack 10 = €299 / 10 × 60-min, 6-month expiry, non-transferable, €35 per unused session, `actual ≤ amount_paid` cap, no refund for consumed, no duplicate refund (4 layers: service pre-flight read + DB CHECK + cascade WHERE filter + outbox UNIQUE), no refund after invalid state, 10/5/0 unused examples, orchestration via the existing approved n8n architecture. **No new SaaS, no new top-level folder, no new env var, no `.env.example` key change, no service-role-key workaround, no RLS weakening, no second Stripe integration.** **The migrations are applied to LOCAL Supabase only.** Remote Supabase, Stripe, Zoom, Calendly, Resend, n8n, Vercel are **operator actions** that remain separately gated. All four quality gates are green: `pnpm type-check` ✓, `pnpm lint` ✓ (1 pre-existing `lib/utils/logger.ts:31` warning unchanged), `pnpm test` ✓ (**922 / 922** across **79** files — +131 new Feature B tests), `pnpm build` ✓ (the 4 new `/api/admin/pack-grants/*` routes + the 2 new `/[locale]/admin/packs` pages registered). Documentation updates landed: `docs/review/PHASE2_SPRINT_FEATURE_B_PACK_REFUND.md` (NEW, 756 lines), `docs/database/Database.md §12.5` (Feature B DB contract), `docs/api/API.md §2.5.4` (Feature B API + state machine + race-safety + outbox rationale), `CHANGELOG.md [1.13.0-phase2-sprint-13-feature-b]` (NEW). **Feature B is UNSTAGED in the working tree**; the final `feat(sprint-13-feature-b): …` commit, the `v1.13.0-phase2-sprint-13-feature-b` tag, and the push to `origin/main` remain pending a separate explicit user authorization.

🟢 **Sprint 13 — Feature A (First free 60-minute session — Pricing Q6) is RELEASED.** Commit `53e07f1` (`feat(sprint-13): complete Feature A free trial session`) and annotated tag `v1.13.0-phase2-sprint-13` (commit `ea13c82` on `origin/main`). The new `apps/web/services/curriculum/free-trial.ts` service, the dashboard banner, and `POST /api/free-trial` route land behind the partial unique index `uq_session_grants_one_trial_per_student`. The `COURSENLIGNE_FREE_TRIAL` coupon row is seeded by its own migration `20260913000003_free_trial_coupon_seed.sql` (moved out of the Feature B migration during preparation so Feature A can ship independently). The `session_grants.is_trial` flag + the partial unique index are added by `20260913000003_free_trial_session_grant.sql`. **No new SaaS, no new top-level folder, no new env var, no `.env.example` key change, no service-role-key workaround, no RLS weakening.** All four quality gates were green at release.

🟢 **Sprint 12 — Feature C Monthly Support (recurring-billing lifecycle — €109 TTC/month, 4 × 60-min/period, no rollover, end-of-period cancellation) is RELEASED.** Two commits shipped Feature C: `c44a9c8` (`feat(sprint-12): complete Sprint 12 close-out — Feature C Monthly Support`) and `21d6f9d` (`fix(sprint-12): complete Feature C Monthly Support integration`, repair tag `v1.12.1-phase2-sprint-12-repair` → `6bab0524e83085188e600ee49e4c7d2a1865cd75`). The integration commit closed the i18n, email-dispatcher, n8n notify template, pricing CTA, and Monthly-first booking-pool gaps left by the initial close-out. **There is no remaining Feature C implementation work** — Feature C is a finished, tagged, released feature of Sprint 12. Schema (forward-only, idempotent, LOCAL Supabase only): `supabase/migrations/20260914000002_monthly_subscription_period_support.sql` adds the `subscription_period_grants` table + 7 columns on `subscriptions` (`current_period_start`, `current_period_end`, `cancel_at_period_end`, `cancelled_at`, `past_due_at`, `suspended_at`, `recovered_at`). Service layer: `apps/web/services/curriculum/monthly-subscriptions.ts` (998 lines) + `apps/web/lib/stripe/subscription-event-handlers.ts` (539 lines). UI: `dashboard/subscription/page.tsx` + `monthly-subscription-card.tsx`. API: `POST /api/subscriptions` + `/api/student/subscription`. Email: three monthly templates (payment_failed, recovered, suspended). Monthly-first booking consumption via `pickSubscriptionPoolFirst` in `session-bookings.ts`. D-1 to D-6 business rules locked and documented in `Database.md §12` + `PHASE2_SPRINT_FEATURE_C_MONTHLY_SUPPORT.md`. **No new SaaS, no new top-level folder, no new env var, no `.env.example` key change, no service-role-key workaround, no RLS weakening beyond the explicit Feature C RLS additions in the migration.** Quality gates were green at Sprint 12 release (979 / 979 tests across 86 files at the integration-fix commit).
