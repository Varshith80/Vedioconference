import { describe, it, expect, vi, beforeEach } from 'vitest';

const mockRequireAdminRoute = vi.fn();
const mockFrom = vi.fn();
vi.mock('@/lib/auth/require-admin-route', () => ({
  requireAdminRoute: mockRequireAdminRoute,
}));
vi.mock('@/lib/supabase/server', () => ({
  createSupabaseServerClientUntyped: () => ({ from: mockFrom }),
}));

const { POST: POST_GRADES } = await import('@/app/api/grades/route');
const { PATCH, DELETE } = await import('@/app/api/grades/[id]/route');
import { NextRequest as NextRequestCtor } from 'next/server';

function makeReq(url: string, body: unknown, method = 'POST'): InstanceType<typeof NextRequestCtor> {
  return new NextRequestCtor(url, {
    method,
    body: JSON.stringify(body),
    headers: { 'content-type': 'application/json' },
  });
}

function makeParams(id: string): Promise<{ id: string }> {
  return Promise.resolve({ id });
}

describe('POST /api/grades', () => {
  beforeEach(() => {
    mockRequireAdminRoute.mockReset();
    mockFrom.mockReset();
  });

  it('returns 401 for anonymous', async () => {
    const { Unauthorized } = await import('@/lib/utils/errors');
    mockRequireAdminRoute.mockRejectedValue(Unauthorized('Sign in.'));
    const res = await POST_GRADES(
      makeReq('http://localhost/api/grades', {
        program_slug: 'lycee',
        slug: 'premiere',
        title: 'Première',
      }) as never,
    );
    expect(res.status).toBe(401);
  });

  it('returns 400 for invalid body (missing program_slug)', async () => {
    mockRequireAdminRoute.mockResolvedValue({ supabase: { from: mockFrom } });
    const res = await POST_GRADES(
      makeReq('http://localhost/api/grades', { slug: 'premiere', title: 'Première' }) as never,
    );
    expect(res.status).toBe(400);
  });

  it('returns 404 when the program does not exist', async () => {
    mockRequireAdminRoute.mockResolvedValue({ supabase: { from: mockFrom } });
    mockFrom.mockReturnValue({
      select() {
        const self: Record<string, unknown> = {};
        self.eq = () => self;
        self.maybeSingle = () => Promise.resolve({ data: null, error: null });
        return self;
      },
    });
    const res = await POST_GRADES(
      makeReq('http://localhost/api/grades', {
        program_slug: 'missing',
        slug: 'premiere',
        title: 'Première',
      }) as never,
    );
    expect(res.status).toBe(404);
  });

  it('returns 201 for a well-formed body', async () => {
    mockRequireAdminRoute.mockResolvedValue({ supabase: { from: mockFrom } });
    let callIdx = 0;
    mockFrom.mockImplementation(() => {
      callIdx++;
      if (callIdx === 1) {
        // programs lookup
        return {
          select() {
            const self: Record<string, unknown> = {};
            self.eq = () => self;
            self.maybeSingle = () => Promise.resolve({ data: { id: 'p1' }, error: null });
            return self;
          },
        };
      }
      // grades insert
      return {
        insert(_rows: unknown) {
          const self: Record<string, unknown> = {};
          self.select = () => self;
          self.single = () =>
            Promise.resolve({
              data: { id: 'g1', slug: 'premiere', title: 'Première' },
              error: null,
            });
          return self;
        },
      };
    });
    const res = await POST_GRADES(
      makeReq('http://localhost/api/grades', {
        program_slug: 'lycee',
        slug: 'premiere',
        title: 'Première',
      }) as never,
    );
    expect(res.status).toBe(201);
    const body = (await res.json()) as { ok: boolean; data: { slug: string } };
    expect(body.data.slug).toBe('premiere');
  });

  it('returns 409 on duplicate (program_id, slug) (23505)', async () => {
    mockRequireAdminRoute.mockResolvedValue({ supabase: { from: mockFrom } });
    let callIdx = 0;
    mockFrom.mockImplementation(() => {
      callIdx++;
      if (callIdx === 1) {
        return {
          select() {
            const self: Record<string, unknown> = {};
            self.eq = () => self;
            self.maybeSingle = () => Promise.resolve({ data: { id: 'p1' }, error: null });
            return self;
          },
        };
      }
      return {
        insert() {
          const self: Record<string, unknown> = {};
          self.select = () => self;
          self.single = () =>
            Promise.resolve({ data: null, error: { code: '23505', message: 'dup' } });
          return self;
        },
      };
    });
    const res = await POST_GRADES(
      makeReq('http://localhost/api/grades', {
        program_slug: 'lycee',
        slug: 'premiere',
        title: 'Première',
      }) as never,
    );
    expect(res.status).toBe(409);
  });
});

describe('PATCH /api/grades/[id]', () => {
  beforeEach(() => {
    mockRequireAdminRoute.mockReset();
    mockFrom.mockReset();
  });

  it('returns 400 for an empty patch body', async () => {
    mockRequireAdminRoute.mockResolvedValue({ supabase: { from: mockFrom } });
    const res = await PATCH(
      makeReq('http://localhost/api/grades/abc', {}) as never,
      { params: makeParams('abc') },
    );
    expect(res.status).toBe(400);
  });

  it('returns 200 on a successful patch', async () => {
    mockRequireAdminRoute.mockResolvedValue({ supabase: { from: mockFrom } });
    mockFrom.mockReturnValue({
      update(_rows: unknown) {
        const self: Record<string, unknown> = {};
        self.eq = () => self;
        self.select = () => self;
        self.single = () =>
          Promise.resolve({
            data: { id: 'g1', slug: 'premiere', title: 'Updated', program_id: 'p1' },
            error: null,
          });
        return self;
      },
    });
    const res = await PATCH(
      makeReq('http://localhost/api/grades/abc', { title: 'Updated' }) as never,
      { params: makeParams('abc') },
    );
    expect(res.status).toBe(200);
  });

  it('returns 404 when the grade does not exist (PGRST116)', async () => {
    mockRequireAdminRoute.mockResolvedValue({ supabase: { from: mockFrom } });
    mockFrom.mockReturnValue({
      update() {
        const self: Record<string, unknown> = {};
        self.eq = () => self;
        self.select = () => self;
        self.single = () =>
          Promise.resolve({ data: null, error: { code: 'PGRST116', message: 'no rows' } });
        return self;
      },
    });
    const res = await PATCH(
      makeReq('http://localhost/api/grades/abc', { title: 'New' }) as never,
      { params: makeParams('abc') },
    );
    expect(res.status).toBe(404);
  });
});

describe('DELETE /api/grades/[id]', () => {
  beforeEach(() => {
    mockRequireAdminRoute.mockReset();
    mockFrom.mockReset();
  });

  it('returns 200 on a successful delete', async () => {
    mockRequireAdminRoute.mockResolvedValue({ supabase: { from: mockFrom } });
    mockFrom.mockReturnValue({
      delete() {
        const self: Record<string, unknown> = {};
        self.eq = () => Promise.resolve({ data: null, error: null });
        return self;
      },
    });
    const res = await DELETE(
      makeReq('http://localhost/api/grades/abc', {}, 'DELETE') as never,
      { params: makeParams('abc') },
    );
    expect(res.status).toBe(200);
  });

  it('returns 409 when the grade still has courses (23503)', async () => {
    mockRequireAdminRoute.mockResolvedValue({ supabase: { from: mockFrom } });
    mockFrom.mockReturnValue({
      delete() {
        const self: Record<string, unknown> = {};
        self.eq = () =>
          Promise.resolve({ data: null, error: { code: '23503', message: 'fk' } });
        return self;
      },
    });
    const res = await DELETE(
      makeReq('http://localhost/api/grades/abc', {}, 'DELETE') as never,
      { params: makeParams('abc') },
    );
    expect(res.status).toBe(409);
  });
});
