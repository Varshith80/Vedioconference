...
// programs list. Edit routes to /admin/programs/[id]. Delete
// opens a `DeleteConfirmDialog` and PATCHes… no, DELETEs the row
// via /api/programs/[id].
// =====================================================================

export interface ProgramRowActionsProps {
  programId: string;
  slug: string;
  title: string;
}
...
export function ProgramRowActions({
  ...
  slug,
  title,
}: ProgramRowActionsProps): React.JSX.Element {
  ...
  return (
    <div className="flex items-center gap-1">
      ...
      <DeleteConfirmDialog
        ...
        expectedSlug={slug}
        resourceLabel={tCommon('resource.program')}
        resourceTitle={title}
        onConfirm={onConfirm}
        submitting={submitting}
      />
      {/* silent "more" icon for the trailing-actions column to keep
          the row visually balanced; the row actions above are
          already explicit. */}
